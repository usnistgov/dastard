...
When a data block is handed to `ProcessSegments(block)` (`data_source.go`), each of the source's `ds.processors` objects is given the appropriate data segment from the block, and `DataStreamProcessor.processSegment` is called on the segment in a goroutine. A `WaitGroup.Add()` is called before each, with a deferred `WaitGroup.Done()`, and all processors as waited on before proceeding. Thus, the processing is done in parallel but all completed before the next step. For some reason, every 20th time through, we now call `dsp.Flush()` to flush any open output files. Then (every time) we `HandleExternalTriggers` and `HandleDataDrop`. If writing is active (not paused), we send a NUMBERWRITTEN message to all listening clients.

The `DataStreamProcessor` contains tons of information: a channel's name and number; the number of samples and pre-trigger samples to be used in making triggered records; a `DataStream`;  a basis and noise-optimal projectors for making linear approximations to a record (for OFF files); info about the last stream index where triggers were made; a `DecimateState`; a `TriggerState`; and a `DataPublisher`.
